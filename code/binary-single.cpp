...
void binary_single(size_t thread_idx, std::string workdir, size_t sim_num, double a_j, double v_inf, double a_s) {
  ...
  for (size_t i = 0; i < sim_num; ++i) {
    ...
    args.add_stop_point_operation(
        [&](auto &ptc) { space::display(out_file, i, jupiter_orbit, solar_orbit, in_orbit, ptc, "\r\n"); });
...
int main(int argc, char **argv) {
  ...
  multi_thread::indexed_multi_thread(th_num, binary_single, output_name, sim_num, a_j, v_inf, a_s);
